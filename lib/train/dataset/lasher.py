...
class LasHeR(BaseVideoDataset):
    ...
    def __init__(self, root=None, split='train', dtype='rgbrgb', seq_ids=None, data_fraction=None, modee_dir=''):
        """
        args:
        ...
        self.dtype = dtype
        self.mod_choose_path = modee_dir
    ...
    def get_sequence_info(self, seq_id):
        ...
        # 读取模态选择伪标签
        mod_choose = np.loadtxt( os.path.join(self.mod_choose_path, seq_path.split('/')[-1]+'.txt') )
        mod_choose = torch.tensor(mod_choose)
        ######
        bbox = self._read_bb_anno(seq_path)
        valid = (bbox[:, 2] > 0) & (bbox[:, 3] > 0)
        visible = valid.clone().byte()
        return {'bbox': bbox, 'valid': valid, 'visible': visible, 'mod_choose': mod_choose}
