...
from lib.test.tracker.bat_fix_ee_pg_erl import BATFixEEPGERLTrack
...
import lib.test.parameter.bat_fix_ee_pg_erl as bat_fix_ee_pg_erl_params
...
def run_sequence(seq_name, seq_home, dataset_name, yaml_name, num_gpu=1, epoch=300, debug=0, script_name='adapter',
                ):
    ...
    for frame_idx, (rgb_path, T_path) in enumerate(zip(RGB_img_list, T_img_list)):
        ...
        if frame_idx == 0:
            ...
            # track
            # image = get_x_frame(rgb_path, T_path, dtype=getattr(params.cfg.DATA,'XTYPE','rgbrgb'))
            region, confidence, out_dict = tracker.track(image)  # xywh
            result[frame_idx] = np.array(region)
            tic_end = cv2.getTickCount()
            if 'layers_v' in out_dict.keys() and out_dict['layers_v'] is not None:
                ee_layer.append([out_dict['layers_v'].cpu(), out_dict['layers_i'].cpu()])
        toc += tic_end - tic
...
if __name__ == '__main__':
    ...
    parser.add_argument('--script_name', type=str, default='bat_fix_ee_pg_erl', help='Name of tracking method(ostrack, adapter, ftuning).')
    parser.add_argument('--yaml_name', type=str, default='fix_ee_pg_erl_stage2_mseloss', help='Name of tracking method.')  # vitb_256_mae_ce_32x4_ep300 vitb_256_mae_ce_32x4_ep60_adapter_i32v21_onlylasher_rgbt
    ...
    if args.script_name == 'bat_fix_ee_pg_erl':
        params = bat_fix_ee_pg_erl_params.parameters(args.yaml_name, args.epoch)
        mmtrack = BATFixEEPGERLTrack(params)  # "GTOT" # dataset_name
