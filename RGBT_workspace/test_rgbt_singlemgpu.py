...
from lib.test.tracker.bat_fix_ee_joint_token import BATFixEEJointTokenTrack
from lib.test.tracker.bat_fix_ee_joint_token_ee import BATFixEEJointTokenEETrack
from lib.test.tracker.bat_fix_ee_distill_medium import BATFixEEDMTrack
from lib.test.tracker.bat_fix_coee_joint_token import BATFixCoEEJointTokenTrack

import lib.test.parameter.bat_fix_coee_joint_token as bat_fix_coee_joint_token_params
...
import lib.test.parameter.bat_fix_ee_joint_token as bat_fix_ee_joint_token_params
import lib.test.parameter.bat_fix_ee_joint_token_ee as bat_fix_ee_joint_token_ee_params
...
def run_sequence(seq_name, seq_home, dataset_name, yaml_name, num_gpu=1, epoch=300, debug=0, script_name='adapter', exit_threshold=[0.65],
    ...
    for frame_idx, (rgb_path, T_path) in enumerate(zip(RGB_img_list, T_img_list)):
        ...
        if frame_idx == 0:
            ...
            elif 'exit_layer' in out_dict.keys():
...
if __name__ == '__main__':
    ...
    parser.add_argument('--yaml_name', type=str, default='rgbt', help='Name of tracking method.')  # rgbt_mimic_fix_ee_stage2_mseloss_discrete2conv,rgbt_mimic_fix_ee_stage2_mseloss_twolinear
    ...
    parser.add_argument('--exit_threshold', default="[0.7]", type=parse_list_arg, help='specific video name')
    ...
    if args.script_name == 'bat':
        ...
        tracker = BAT_RGBT(tracker=mmtrack)
    elif args.script_name == 'bat_fix_ee_joint_token':
        params = bat_fix_ee_joint_token_params.parameters(args.yaml_name, args.epoch)
        mmtrack = BATFixEEJointTokenTrack(params)  # "GTOT" # dataset_name
        tracker = BAT_RGBT(tracker=mmtrack)
    elif args.script_name == 'bat_fix_ee_joint_token_ee':
        params = bat_fix_ee_joint_token_ee_params.parameters(args.yaml_name, args.epoch)
        mmtrack = BATFixEEJointTokenEETrack(params)  # "GTOT" # dataset_name
        tracker = BAT_RGBT(tracker=mmtrack)
    elif args.script_name == 'bat_fix_coee_joint_token':
        params = bat_fix_coee_joint_token_params.parameters(args.yaml_name, args.epoch)
        mmtrack = BATFixCoEEJointTokenTrack(params)  # "GTOT" # dataset_name
        tracker = BAT_RGBT(tracker=mmtrack)
