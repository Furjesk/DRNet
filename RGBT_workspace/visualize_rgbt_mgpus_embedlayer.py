...
def run_sequence(seq_name, seq_home, dataset_name, yaml_name, num_gpu=1, epoch=300, debug=0, script_name='adapter', exit_threshold=0.65):
    ...
    for frame_idx, (rgb_path, T_path) in enumerate(zip(RGB_img_list, T_img_list)):
        ...
        if frame_idx == 0:
            ...
            heatmap = FeatureMapVisible(out_dict['token_embedding'][0][:,64:320].transpose(-1,-2).reshape(-1,768,16,16), None)
            axs[0][3].imshow(heatmap[:,:,::-1])  
            axs[0][3].set_title('RGB embed x')  
            axs[0][3].axis('off')  
            heatmap = FeatureMapVisible(out_dict['token_embedding'][1][:,64:320].transpose(-1,-2).reshape(-1,768,16,16), None)
            axs[0][4].imshow(heatmap[:,:,::-1])  
            axs[0][4].set_title('TIR embed x')  
            axs[0][4].axis('off')  

            heatmap = FeatureMapVisible(out_dict['token_embedding'][0][:,:64].transpose(-1,-2).reshape(-1,768,8,8), None)
            axs[1][0].imshow(heatmap[:,:,::-1])  
            axs[1][0].set_title('RGB embed z')  
            axs[1][0].axis('off')  
            heatmap = FeatureMapVisible(out_dict['token_embedding'][1][:,:64].transpose(-1,-2).reshape(-1,768,8,8), None)
            axs[1][1].imshow(heatmap[:,:,::-1])  
            axs[1][1].set_title('TIR embed z')  
            axs[1][1].axis('off')  
...
if __name__ == '__main__':
    ...
    parser.add_argument('--script_name', type=str, default='bat_fix_ee', help='Name of tracking method(ostrack, adapter, ftuning).')
    parser.add_argument('--yaml_name', type=str, default='rgbt_mimic_fix_ee_stage2_mseloss', help='Name of tracking method.')
    ...
    parser.add_argument('--exit_threshold', default=[0.7], type=list, help='specific video name')
