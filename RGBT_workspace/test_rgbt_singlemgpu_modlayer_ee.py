...
from lib.test.tracker.bat_mod_layer_ee import BATModLayerEETrack
from lib.test.tracker.bat_mod_layer_coee_lu import BATModLayerCoEELuTrack

import lib.test.parameter.bat_mod_layer_ee as bat_mod_layer_ee_params
...
def run_sequence(seq_name, seq_home, dataset_name, yaml_name, num_gpu=1, epoch=300, debug=0, script_name='adapter', exit_threshold=[0.65],
    ...
    save_ee_path = f'./RGBT_workspace/results/{dataset_name}/' + save_name +  '/' + seq_txt + '_mod_ee.txt'
    ...
    for frame_idx, (rgb_path, T_path) in enumerate(zip(RGB_img_list, T_img_list)):
        ...
        if frame_idx == 0:
            ...
            tic_end = cv2.getTickCount()
            if 'mod_choose' in out_dict.keys() and out_dict['mod_choose'] is not None:
                mod_ee_list.append(out_dict['mod_choose'].max(dim=1)[1].item())
    ...
    if not debug:
        ...
        np.savetxt(save_ee_path, np.array(mod_ee_list), fmt='%d')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--script_name', type=str, default='bat_mod_layer_coee_lu', help='Name of tracking method(ostrack, adapter, ftuning).')
    parser.add_argument('--yaml_name', type=str, default='rgbt_20_only_layeree_token_pred_train_all_mod_hasmimic_task_toy3', help='Name of tracking method.')  # rgbt_mimic_fix_ee_stage2_mseloss_discrete2conv,rgbt_mimic_fix_ee_stage2_mseloss_twolinear
    ...
    parser.add_argument('--epoch', default=18, type=int, help='epochs of ckpt')
    ...
    parser.add_argument('--exit_threshold', default="[0.75]", type=parse_list_arg, help='specific video name')
    ...
    if args.script_name == 'bat_mod_layer_ee':
        params = bat_mod_layer_ee_params.parameters(args.yaml_name, args.epoch)
        mmtrack = BATModLayerEETrack(params)  # "GTOT" # dataset_name
        tracker = BAT_RGBT(tracker=mmtrack)
    elif args.script_name == 'bat_mod_layer_coee_lu':
        params = bat_mod_layer_coee_lu_params.parameters(args.yaml_name, args.epoch)
        mmtrack = BATModLayerCoEELuTrack(params)  # "GTOT" # dataset_name
        tracker = BAT_RGBT(tracker=mmtrack)
