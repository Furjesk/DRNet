...
class FocalLoss(nn.Module, ABC):
    ...
    def forward(self, prediction, target, reduction='mean'):
        ...
        if reduction == 'mean':
            ...
            B = positive_index.shape[0]
            num_positive = positive_index.float().reshape(B,-1).sum(-1)
            positive_loss = positive_loss.reshape(B,-1).sum(-1)
            negative_loss = negative_loss.reshape(B,-1).sum(-1)

            if 0 in num_positive:
                loss = torch.zeros_like(num_positive)
                for i in range(B):
                    if num_positive[i] == 0:
                        loss[i] = -negative_loss[i]
                    else:
                        loss[i] = -(positive_loss[i] + negative_loss[i]) / num_positive[i]
                # loss = -negative_loss
            else:
                loss = -(positive_loss + negative_loss) / num_positive
